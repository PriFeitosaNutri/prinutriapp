...
const WelcomeScreen = ({ user, onContinue }) => {
  ...
  const handleContinue = () => {
    onContinue(); // This will just update has_seen_welcome to true
  };
        ...
        <Card className="shadow-2xl overflow-hidden">
          ...
          <CardContent className="p-6 md:p-8 space-y-8">
            ...
            <div className="text-center space-y-4">
              <h3 className="text-xl font-semibold text-primary">Próximo Passo: Sua Anamnese</h3>
              <p className="text-muted-foreground">Vamos precisar de algumas informações para criar seu plano personalizado. Clique abaixo para começar.</p>
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button onClick={handleContinue} size="lg" className="w-full md:w-auto bg-accent hover:bg-accent/90 text-accent-foreground text-lg py-3 px-8 shadow-lg">
                  <FileText className="w-6 h-6 mr-3" />Preencher Anamnese
                </Button>
              </motion.div>
            </div>
          </CardContent>
