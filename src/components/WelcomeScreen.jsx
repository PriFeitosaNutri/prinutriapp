  ...
  const fetchVideoUrl = useCallback(async () => {
    try {
      ...
      if (setting && setting.url) {
        setVideoUrl(setting.url);
      } else {
        setVideoUrl("https://www.youtube.com/embed/I3ejjYmKv1Y?si=0ibGUoKg85mu2ZV_&autoplay=1"); // Fallback
      }
    } catch (error) {
      toast({
          ...
          <CardContent className="p-6 md:p-8 space-y-8">
            ...
            <div className="text-center space-y-4">
              ...
              <motion.div whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }}>
                <Button onClick={handleScheduleChat} size="lg" className="w-full md:w-auto bg-accent hover:bg-accent/90 text-accent-foreground text-lg py-3 px-8 shadow-lg">
